<%
	...
	<%InsuranceBean b= (InsuranceBean)session.getAttribute("insurancetoupdate"); %>
<!DOCTYPE html >
<html >
<head>

<title>Fast Quote Insurance | Update Insurance</title>
...
function validateForm1()
{
	...
	// alert(d);
	 if (d=null || d=="")
	   {
	   alert("startdate must be filled out 222");
	   return false;
	   }
...
<form name="input" action="UpdateInsuranceValuesServlet" method="post"  onsubmit="return validateForm1()" onreset="return validateForm2()" >
<input type="hidden" name="addinsurance" value="addinsurance"></input>
<table cellspacing="50px">

<tr><td>
Type of Insurance</td><td>:</td><td>
<select name="typeofinsurance" id="dropbox" onchange="check_option(this.value)" disabled>
<option value="0">Choose Your Option</option>
<%
	String[] options={"Body-injury","vehicle-damage","liability"};
	String flag="";
	for(int i=0;i<3;i++){
		flag="";
		if(options[i].equals(b.getTypeofinsurance()))
				flag="selected";
%>
<option value="<%=options[i]%>" <%=flag %>><%=options[i] %> coverage</option>
<%}
	String vehicleid="";
	if(b.getTypeofinsurance().equals(options[0])){
		flag="disabled";
	}
	else{
...
<tr><td>Vehicle id</td><td>:</td><td><input type="text" id="textbox" name="vehicleid" <%=flag%> value="<%=vehicleid %>" ></input></td></tr>
<tr><td>Coverage amount</td><td>:</td><td><input type="text" id="textbox" name="coverage" value=<%=b.getCoverage()%>></td></tr>
<tr><td>Duration</td><td>:</td><td><input type="text" name="noofmonths"  id="textbox" value="<%=b.getNoofmonths()%>"></td></tr>
<tr><td>Start Date</td><td>:</td><td><input type="date" placeholder="yyyy-mm-dd" id="textbox" value=<%=b.getStartdate() %> id="startdate" name="startdate" ></td></tr>
<tr style="text-align:center;"><td><input type="submit" id="button1" value="Submit"  ></td><td></td>
